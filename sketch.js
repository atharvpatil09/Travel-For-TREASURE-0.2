...
var reStartImg, keyImg,coinsImg,keysGroup,playImg,homeImg,boy2Img, backgroundMusic1, backgroundMusic2, bonus,gameNamePlate;
var score = 0;
var keys = 0;
...
function preload(){
  ...
  gameNamePlate = loadImage("gameName.jpg");
...
function draw(){
   ...
   if(gameState===START) 
  {
   ...
   push();
    ...
    rect(95,40,600,200);
    textSize(20)
    text("--STORY OF THE GAME--",280,70)
    pop();
    
    textSize(20);
    fill("pink");
    text("Ones there was a boy named Raghu. He cames to know that there ",100,100);
    text("is a big treasure in the forest. That treasure is located in a fort in.",100,120);
    text("Raghu needs so many keys to unlock that forts door. So he have to ",100,140);
    text("cover so much distance in that forest. But its not easy, ",100,160);
    text("because there are so many guards on his way, so he have to",100,180);
    text("escape from that guards then he can reach that treasure.",100,200);
    
    stroke("yellow");
    fill("black");
    rect(95,250,600,100);
    text("HOW TO PLAY",320,270);
    
    stroke("blue");
    fill("red");
    text("You have to escape from the guards ,to collect the coins and to ",100,300);
    text("collect the keys in the game You have to press-> UP_ARROW key " ,100,320);
    text("so you can jump and escape from guards and collect coins & keys",100,340)
    ...
    image(keyImg,50,900);
    scale(1.5)
    image(playImg,750,850);
    
    scale(0.9);
    image(gameNamePlate,680,5);
    pop();
